...
- **当面はC# / .NETで保守する。** 画面、ファイルアクセス、検査制御、解析、結果表示をC#で改善する。WPFのXAML、既存のWindows API呼び出し、開発・検証用PowerShellは継続する。
- **Rustは必要性が確認できた解析部分だけ検討する。** 計測で判明した負荷、または複雑な形式への具体的な利点を根拠とし、C#での改善と保守コストを比較する。将来のためだけのDLLや抽象化は追加しない。
- 採用時は、C#が検証して読み取ったサイズ制限付きバイト列を小さな解析DLLへ渡す。同一プロセスのDLLを隔離とは扱わず、既存の資源上限と保護を維持する。詳細は[実装言語の設計記録](docs/implementation-language.md)を参照する。
