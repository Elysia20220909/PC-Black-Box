...
以下は公開済みプレビューとは別の説明です。トップレベルのOLEはストレージ木とストリーム本文を上限内で調べますが、VBA本体・MSIテーブルは解読しません。破損構造と未解読の内容は完全性の限界として残し、ZIP内のOLEは開かず`INCOMPLETE`とします。

Microsoft Defenderの保護状態・既存履歴は読み取り専用で、スキャンや隔離は開始しません。`--protection-status`のSandboxie Plus / QEMU登録情報は、隔離の有効性を証明しません。[Defender連携](docs/defender-readonly.md)

DiEの補助分類は別プロセスのAppContainerで行います。単一ファイル・64 MiB以下が対象で、通常の本体ビルドだけでは統合ランチャーやエンジンは揃いません。対象は実行せず、分類を安全判定やアプリ全体の隔離とは扱いません。[構成と検証範囲](docs/die-integration.md) / [ウィンドウ操作の実装](docs/window-controls.md)

以下の表と技術リファレンスは記載された旧版の記録であり、統合ソースの機能一覧ではありません。
